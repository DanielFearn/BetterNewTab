...
var btns = {
	...
	save_buttons: function(){
		var button_string = JSON.stringify(this.buttons);
	...
	generate_buttons: function(){
		...
		for(var title in this.buttons){
			...
			$('#main_container').append('<a href="#" class="large_btn link_btn" title="'+desc+'">'+title+this.button_delete+'</a>');
		}
		$('#main_container').append(this.new_btn_button);
	...
	btn_click: function(button){
		...
		var tabs = this.buttons[title].URLs.split(',');
	...
	set_click_events: function(){
		...
		$('.button_delete').click(function(){
			btns.delete_button($(this).parent());
		});
